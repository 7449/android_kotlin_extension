...
fun String.milliseconds(format: SimpleDateFormat = DEFAULT_FORMAT): Long {
    return runCatching { format.parse(this)?.time ?: -1 }.getOrNull() ?: -1
}

fun String.date(format: SimpleDateFormat = DEFAULT_FORMAT): Date {
    return Date(milliseconds(format))
}
